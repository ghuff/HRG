...
public class MedusaCubeHandler{
 ...
 public void sendCubeThetaPhi(float theta,float phi,int input){
     if(input == 0){
       theta = radians(theta);
       phi = radians(phi);
     }
 ...
 public void sendCubeElementLocations(float xpos, float ypos, float zpos, int antNum){
   sendDynamicCoordsActivate();
   sendCubeCoordsMessage(xpos,ypos,zpos,antNum);
 }
 ...
 public void sendCubePrintPhases(){
   int start = 255;
    int packLength = 1;
    int frameNum = 96; //the frame type
    int frameID = 1;
    int dummy = 1;
    int checksum = start + packLength + frameNum + frameID + dummy;
    checksum = checksum % 256;
